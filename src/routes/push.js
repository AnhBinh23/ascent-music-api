...
// ─── Gửi push tới danh sách user (dùng cho broadcast) ────────────────────────
...
// ─── Nhắc lịch học tự động (gọi mỗi 30 phút) ────────────────────────────────
router.post('/remind-schedule', async (req, res) => {
  try {
    ...
    for (const schedule of schedules) {
      ...
      await sendPushToUser(schedule.student_id, JSON.parse(payload));
      sent++;
    }
...
// ─── Helper: gửi push tới 1 user (dùng nội bộ) ───────────────────────────────
